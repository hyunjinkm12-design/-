...
export const DeliverablesModal: React.FC<DeliverablesModalProps> = ({ task, onClose, onSave }) => {
  ...
  const handleAddDeliverable = () => {
    const newDeliverableName = prompt("Enter the name for the new deliverable group (e.g., 'Design Mockups'):");
  ...
  const handleFileChange = async (event: React.ChangeEvent<HTMLInputElement>) => {
    if (!event.target.files || !activeDeliverableId) return;
          ...
          <div className="space-y-6">
            {deliverables.length === 0 ? (
                <div className="text-center py-10 border-2 border-dashed border-gray-300 rounded-lg">
                    ...
                    <p className="text-sm text-gray-400">Click 'Add Deliverable Group' to get started.</p>
                </div>
                        ...
                        <tbody>
                          {deliverable.versions.sort((a,b) => b.version - a.version).map(version => (
                            <tr key={version.version} className="border-t border-gray-200 hover:bg-gray-100">
                                ...
                                <td className="p-2 text-blue-600 font-medium">{version.fileName}</td>
